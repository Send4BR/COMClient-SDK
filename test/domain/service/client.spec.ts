...
tap.test('should send a email message using fake provider', async (t) => {
  ...
  tap.test('should send a SMS message using fake provider', async (t) => {
    ...
    t.equal(FakerMessageSender.messages.length, 1)
    t.equal((FakerMessageSender.messages[0] as SMSData).message.text, 'Loja da Ana: E isso ai Como a gente achou que ia ser A vida tao simples e boa Qu ... Veja mais em: https://lojadaana.delivery.after.sale/order/1234 PEDIDO #1234')
